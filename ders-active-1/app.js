...
//   fiyatı 30 ile 100 arası kaç adet ürün vardır?

const filteredProducts = products.filter(
    (product) => product.unitPrice < 100 && product.unitPrice > 30
);
const length = filteredProducts.length; //22
...
// ismi a harfi ile biten ürünleri console a yazdır

products.map((product) => {
    if (product.name.endsWith("a")) {
        console.log(product.name);
    }
});

// id si 10 olanın ismini yazdır

products.map((product) => product.id === 10 ? console.log(product.name) : null);

//  discoutinuted ı false olan kaç ürün vardır

const filterdByDiscontinued = products.filter(product => product.discontinued === false)
console.log(filterdByDiscontinued.length) // 68

// en pahalı ürünün adını console a yazdır

const higherPrice = products.reduce((prev, current) => prev.unitPrice > current.unitPrice ? prev : current)
console.log(higherPrice.unitPrice) // 263.5

// en ucuz ürünün adını console a yazdır

const lowerPrice = products.reduce((prev, current) => prev.unitPrice > current.unitPrice ? prev : current)
console.log(lowerPrice.unitPrice) // 263.5
